...
import { MdSearch, MdAnalytics,  MdMenuBook, MdPerson, MdClass, MdBadge } from "react-icons/md";
...
function Analytics() {
  ...
  const updateDate = (date: DateRange) => { setDate(date) };
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!studentId.startsWith("STU")) {
      toast(`❌ Please enter a valid student id.`);
      return;
    }

    if (studentId && date.from && date.to) {
      mutate({ studentId: studentId.split("STU")[1], date });
    } else {
      toast(`❌ Please enter student id and date.`);
    }
  };
  ...
  const { first_name, middle_name, last_name, class: className, id } = data?.student || {};
      ...
      <div className="max-w-7xl mx-auto bg-white border border-gray-200 rounded-3xl overflow-hidden shadow-sm">
        <div className="grid lg:grid-cols-[340px_1fr] min-h-[750px]">
          ...
          <div className="bg-gradient-to-b from-emerald-50 to-white border-b lg:border-b-0 lg:border-r border-gray-100 p-5 sm:p-7">
            ...
            <div className="flex items-center gap-3 mb-8">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400 mt-1">Student issued books data</p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="bg-white border border-gray-100 rounded-2xl p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs uppercase tracking-wider text-gray-400">Student</p>
              ...
              <div className="bg-white border border-gray-100 rounded-2xl p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs uppercase tracking-wider text-gray-400">Class</p>
                    <p className="text-sm font-semibold text-gray-700 mt-1">{className || "--"}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-100 rounded-2xl p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs uppercase tracking-wider text-gray-400">Student ID</p>
                    <p className="text-sm font-semibold text-gray-700 mt-1">
                      {id ? `STU${id}` : "--"}
                    </p>
                  </div>
              ...
              <div className="bg-emerald-600 rounded-3xl p-5 text-white mt-8">
                ...
                <p className="text-sm text-emerald-100 mt-2 leading-relaxed">
                  Track student book issue history with date-based filtering and organized records.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="p-5 sm:p-7 flex flex-col">
            ...
            {/* TITLE */}
            <div className="mb-8">
              <h2 className="text-2xl font-semibold text-gray-800">Student Analytics</h2>
              <p className="text-sm text-gray-400 mt-1">
                Search issued books by student ID and date range
              </p>
            </div>
              ...
              <div>
                ...
                <div className="relative">
                  {/* <div className="absolute left-4 top-1/2 -translate-y-1/2 z-10">
                    <MdDateRange className="text-xl text-gray-400 border-blue-500" />
                  </div> */}
                ...
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[750px]">
                    <thead className="bg-emerald-50 border-b border-gray-200">
                      <tr>
                        {["Book", "Issued Date", "Author", "Publisher", "ISBN"].map((col) => (
                          <th  key={col}
                            className="text-left px-5 py-4 text-xs uppercase tracking-wider text-gray-500 font-semibold"
                          >
                            {col}
                          </th>
                        ))}
                    ...
                    <tbody>
                      {data.books.map((book, index) => (
                        <tr
                          key={book.id}
                          className={`border-b border-gray-100 hover:bg-gray-50 transition-all ${index % 2 === 0 ? "bg-white" : "bg-gray-50/40"
                            }`}
                        >
                          ...
                          <td className="px-5 py-4 text-sm text-gray-600">
                            {getFormattedDate(book.student_books?.[0]?.created_at)}
                          </td>
                          <td className="px-5 py-4 text-sm text-gray-600">{book.author}</td>
                          <td className="px-5 py-4 text-sm text-gray-600">{book.publisher}</td>
                          <td className="px-5 py-4 text-sm font-medium text-gray-700">{book.isbn}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
