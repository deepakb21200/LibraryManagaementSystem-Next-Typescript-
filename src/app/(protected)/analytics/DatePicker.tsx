...
import { format } from "date-fns";//Date ko user-friendly text me convert karna format(date.from, "LLL dd, y")
...
function DatePicker({ date, updateDate }: DatePickerProps) {
  ...
  return (
    <div className="relative w-full">
      ...
      <button type="button" onClick={() => setShow(!show)}
        className="w-full h-12 rounded-xl border border-gray-200 bg-gray-50 pl-12 pr-4 text-sm text-left
         text-gray-700 outline-none transition-all hover:bg-white focus:border-emerald-400">
        {/* {date?.from ?
      ...
      {/* DATE PICKER */}
      {show && (
        // <div className="absolute top-14 right-0 left-0 z-50 bg-white border border-gray-200 rounded-2xl shadow-xl p-4 w-full">
        <div className="absolute top-14 left-0 right-0 z-50 bg-white border border-gray-200 rounded-2xl shadow-xl p-4 w-full 
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => updateDate({ ...date, from: new Date(e.target.value) })}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  updateDate({ ...date, to: new Date(e.target.value) })
                }
                className="w-full h-11 rounded-xl border border-gray-200 bg-gray-50 px-4 text-sm outline-none focus:border-emerald-400 focus:bg-white"
              />
            </div>
